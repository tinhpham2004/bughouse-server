...
serviceDemandSchema.statics.getNewestservice = async (serviceId) => {
  const serviceDemands = await ServiceDemand.find({ service: serviceId }).sort({
    _id: -1,
  });
...
serviceDemandSchema.statics.getLastService = async (serviceId, lastMonth) => {
  const serviceDemand = await ServiceDemand.findOne({
    service: serviceId,
    atMonth: lastMonth,
  }).populate({
    path: "service",
...
serviceDemandSchema.statics.getPresentService = async (
  serviceId,
  presentMonth
) => {
  const serviceDemand = await ServiceDemand.findOne({
    service: serviceId,
    atMonth: presentMonth,
  }).populate({
    path: "service",
    select: "name description basePrice unit",
  });
