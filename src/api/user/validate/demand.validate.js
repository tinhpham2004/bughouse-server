...
const ServiceDemandValidate = {
  ...
  validateDemandInfo: async (serviceId, serviceDemandInfo) => {
    if (!(serviceId && serviceDemandInfo)) { throw new ArgumentError("validate service demand ==>"); }
    ...
    if (!service) { throw new NotFoundError("validate service demand => service"); }
    const { unit } = service;
    ...
    if (newestDemand) { oldIndicator = newestDemand.newIndicator; }
  ...
  validateUpdateDemandInfo: (demandInfo) => {
    if (!demandInfo) { throw new ArgumentError("validate demand==> demand info "); }
  ...
  validateCreateDemandForRoom: ({ room, period, dateRent }) => {
    ...
    if (!expiredDate) { throw new MyError("validate demand ==> expired date invalid"); }
    const { services } = room;
    if (!services) { throw new MyError("validate demand => services invalid"); }
